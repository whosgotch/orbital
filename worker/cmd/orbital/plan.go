...
// planRepo runs a repo-level planning pass: the AI reads the repo, writes a
// plan for the goal in the chosen format, and proposes the tasks to carry it
// out. Nothing materializes until the plan is approved (see approve-plan).
...
func planRepo(ctx context.Context, args []string, stdout io.Writer) error {
	...
	if _, err := service.PlanRepo(ctx, repository.ID, goal, domain.PlanFormat(format)); err != nil {
		return err
	}
